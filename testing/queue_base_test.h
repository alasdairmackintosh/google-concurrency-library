    ...
    Queue* b )
{
    for ( int i = 1; i <= count; ++i ) {
        ...
        ASSERT_EQ(queue_op_status::success, b->try_pop(popped));
    ...
    queue_front<int> ft )
{
    for ( int i = 1; i <= count; ++i ) {
        int popped;
        ASSERT_FALSE(ft.is_empty());
        ASSERT_EQ(queue_op_status::success, ft.try_pop(popped));
        ASSERT_EQ(i * multiplier, popped);
    }
    ASSERT_TRUE(ft.is_empty());
}
...
{
    int result;
    ASSERT_EQ(queue_op_status::empty, q->try_pop(result));
    ASSERT_EQ(queue_op_status::success, q->try_push(1));
    ASSERT_EQ(queue_op_status::success, q->try_pop(result));
    ASSERT_EQ(1, result);
...
{
    int result;
    ASSERT_EQ(queue_op_status::empty, ft.try_pop(result));
    ASSERT_EQ(queue_op_status::success, bk.try_push(1));
    ASSERT_EQ(queue_op_status::success, ft.try_pop(result));
    ASSERT_EQ(1, result);
    ASSERT_TRUE(ft.is_empty());
}
    ...
    Queue* q )
{
    ...
    ASSERT_EQ(queue_op_status::full, q->try_push(count + 1));
    // This assert might fail if try_pop doesn't return success.
    int val;
    while (q->try_pop(val) != queue_op_status::success) {}
    ASSERT_EQ(1, val);
    ASSERT_EQ(queue_op_status::success, q->try_push(count + 1));
    for ( int i = 2; i <= count + 1; ++i ) {
        int popped;
        ASSERT_FALSE(q->is_empty());
        ASSERT_EQ(queue_op_status::success, q->try_pop(popped));
        ASSERT_EQ(i, popped);
    }
    ASSERT_TRUE(q->is_empty());
}
    ...
    queue_front<int> ft)
{
    ...
    ASSERT_EQ(queue_op_status::full, bk.try_push(count + 1));
    ASSERT_EQ(1, ft.value_pop());
    ASSERT_EQ(queue_op_status::success, bk.try_push(count + 1));
    for ( int i = 2; i <= count + 1; ++i ) {
        int popped;
        ASSERT_FALSE(ft.is_empty());
        ASSERT_EQ(queue_op_status::success, ft.try_pop(popped));
    ...
    queue_front<int> ft )
{
    ...
    bk.close();
    ASSERT_TRUE(bk.is_closed());
    ASSERT_EQ(queue_op_status::closed, bk.try_push(42));
    seq_try_drain(count, 1, ft);
    ASSERT_TRUE(ft.is_closed());
    int popped;
    queue_op_status result = ft.try_pop(popped);
    ASSERT_EQ(queue_op_status::closed, result);
}
    ...
    queue_base<int>& queue )
{
    ...
    std::thread t1(std::bind(drain, count, 1, &queue));
    std::thread t2(std::bind(fill, count, 1, &queue));
...
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(try_drain, count, 1, &queue));
    std::thread t2(std::bind(try_fill, count, 1, &queue));
    // Join in order of expected completion.
    // Closing as we go to stop the other thread.
    t2.join();
    queue.close();
...
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(drain, count, 2, &tail));
    std::thread t2(std::bind(filter, &head, &tail, twice));
    std::thread t3(std::bind(fill, count, 1, &head));
    // Join in order of expected completion.
...
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(try_drain, count, 2, &tail));
    std::thread t2(std::bind(filter, &head, &tail, twice));
    std::thread t3(std::bind(try_fill, count, 1, &head));
    // Join in order of expected completion.
    // Closing as we go to stop the other threads.
    t3.join();
    head.close();
...
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(drain_pos_neg, 2 * count, 2, &tail));
    std::thread t2(std::bind(filter, &head, &tail, twice));
    std::thread t3(std::bind(fill, count, 1, &head));
    std::thread t4(std::bind(fill, count, -1, &head));
    // Join in order of expected completion.
    // Closing as we go to stop the other threads.
    t4.join();
    t3.join();
...
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(try_drain_pos_neg, 2 * count, 2, &tail));
    std::thread t2(std::bind(filter, &head, &tail, twice));
    std::thread t3(std::bind(try_fill, count, 1, &head));
    std::thread t4(std::bind(try_fill, count, -1, &head));
    // Join in order of expected completion.
    // Closing as we go to stop the other threads.
    t4.join();
    t3.join();
...
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(drain_any, count, -2, &tail));
    std::thread t2(std::bind(drain_any, count, 2, &tail));
    std::thread t3(std::bind(filter, &head, &tail, twice));
    std::thread t4(std::bind(filter, &head, &tail, twice));
    std::thread t5(std::bind(fill, count, 1, &head));
    std::thread t6(std::bind(fill, count, -1, &head));
    // Join in order of expected completion.
    // Closing as we go to stop the other threads.
    t6.join();
    t5.join();
    ...
    queue_base<int>& tail )
{
    // Start drain first for extra testing of waiting
    std::thread t1(std::bind(drain_any, count, -2, &tail));
    std::thread t2(std::bind(try_drain_any, count, 2, &tail));
    std::thread t3(std::bind(filter, &head, &tail, twice));
    std::thread t4(std::bind(filter, &head, &tail, twice));
    std::thread t5(std::bind(fill, count, 1, &head));
    std::thread t6(std::bind(try_fill, count, -1, &head));
    // Join in order of expected completion.
    // Closing as we go to stop the other threads.
    t6.join();
    t5.join();
