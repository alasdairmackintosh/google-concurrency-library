...
struct LockedTask {
  LockedTask(std::mutex* lock) : block_lock(lock) {}

  void run() {
    std::unique_lock<std::mutex> ul(*block_lock);
    ++count;
  }
...
TEST(SerialExecutorTest, ShutdownTest) {
  ...
  std::thread exec_deleter(std::bind(&delete_executor, exec));
